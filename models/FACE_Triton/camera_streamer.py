...
from FACE_Triton_v2.FACE_Triton.client_triton import Flipface
import os, sys, datetime
import numpy as np
import os.path as osp
import cv2
from FACE_Triton_v2.FACE_Triton.Pose_processing.face import SCRFD
import insightface
...
from Search_engine.faiss_helpers import FaissApps1
...
def cameraStream(url,connect_to):
        ...
        for i in range(bboxes.shape[0]):
            ...
            if face.shape <= (128, 128, 3):
                ...
                if mask == False :
                    ...
                    idx,name=con.search(Embeding_Face,1)
                    ...
                    idx,name=con.search(Embeding_mask,1)
                    #cv2.putText(frame,str(name["name"]),(x2-30,y2-30),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2,cv2.LINE_AA)
                    # if name["flag"]== True:
                    #     print(name["name"])
                    #print(name)
    ...
    #             # else:
    #             #     print(name["name"])
    #             con.search(Embeding_Face,1)
    #             #print(name)
    #         else : 
    #             Embeding_mask=model.Embeding_Face_mask(frame)
    #             con.search(Embeding_mask,1)
